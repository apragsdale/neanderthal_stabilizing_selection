...
n = 300
fss = [init_fs(3 * n, gamma, theta) for gamma in gammas]

for i, fs in enumerate(fss):
    fss[i] = fs.split(0, 2 * n, n)
...
for i, fs in enumerate(fss):
    fss[i] = fs.pulse_migrate(0, 1, n, 0.05)
    print(i)

T = (250000 - 60000) / 25 / 2 / Ne
for a, fs in zip(a_list, fss):
    gamma = get_gamma(a, EVG, Ne, VS=VS)
    fs.integrate(nus, T, theta=theta, overdominance=gamma)
    print(a)
...
T_bottle = 60000
T_admix = 50000
r = np.log(N_Eur / N_bottle) / (T_bottle / 25 / 2 / Ne)
nu_func = lambda t: [N_bottle / Ne * np.exp(r * t), 0.1]
for a, fs in zip(a_list, fss):
    gamma = get_gamma(a, EVG, Ne, VS=VS)
    fs.integrate(
        nu_func, (T_bottle - T_admix) / 25 / 2 / Ne, theta=theta, overdominance=gamma
    )
    print(a)
...
for i in range(len(fss)):
    ...
    fss_seg.append(fs_seg.admix(1, 0, n, 0.02))
    fss_intro.append(fs_intro.admix(1, 0, n, 0.02))
    print("admixed", i)

fss_seg_copy = copy.deepcopy(fss_seg)
fss_intro_copy = copy.deepcopy(fss_intro)
...
VG_list = [get_VG(fs.marginalize([1]), a) for fs, a in zip(fss, a_list)]
...
def get_h2s(fss_intro, fss_seg, SD):
    ...
    h2_intro_bin_sn = [h / m for h, m in zip(h2_intro_bin, M_intro_bin)]
...
for SD in [0.01, 0.02, 0.05]:
    VG_tot = gaussian_des(a_list, VG_list, SD)
    print(SD, VG_tot)
    all_data[SD] = {"VG_init": VG_tot}
    all_data[SD]["traj"] = {0: get_h2s(fss_intro, fss_seg, SD)}
    print(SD, all_data[SD]["traj"][0])

N_curr = N_bottle / Ne * np.exp(r * (T_bottle - T_admix) / 25 / 2 / Ne)

for i in range(100):
    nu_func = lambda t: [N_curr * np.exp(r * t)]
    for a, fs in zip(a_list, fss_seg):
        gamma = get_gamma(a, EVG, Ne, VS=VS)
        fs.integrate(nu_func, T, theta=theta, overdominance=gamma)
    for a, fs in zip(a_list, fss_intro):
        gamma = get_gamma(a, EVG, Ne, VS=VS)
        fs.integrate(nu_func, T, theta=0, overdominance=gamma)
    N_curr = nu_func(T)[0]
    for SD in [0.01, 0.02, 0.05]:
        all_data[SD]["traj"][(i + 1) * T] = get_h2s(fss_intro, fss_seg, SD)
        print(SD, N_curr, all_data[SD]["traj"][(i + 1) * T])
