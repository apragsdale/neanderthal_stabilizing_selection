...
def set_up_demographic_model(Ne=10000):
    ...
    T_H_to_N = 250000
    T_N_to_H = 50000
    b = demes.Builder(time_units="years", generation_time=gen)
    b.add_deme(
        "Human",
        epochs=[
...
def run_sim(
    L, g, mu, spacing=2e6, win_size=100000, mean=-0.0002, shape_param=1000, burnin=10
):
    # effect sizes drawn from a Gaussian, placed 2 Mb apart
    left = (spacing - win_size) / 2
    right = (spacing + win_size) / 2
    sregions = [
        fwdpy11.GammaS(i * spacing + left, i * spacing + right, 1, mean, shape_param)
        #fwdpy11.GaussianS(i * spacing + left, i * spacing + right, 1, SD)
    ...
    sample_times = [simlen - 250000 // 25 - 1, simlen - 120000 // 25, simlen - 45000 // 25]
...
if __name__ == "__main__":
    ...
    mu = float(sys.argv[2])
    print(f"Running mu={mu}, with seed={seed}")
    ...
    print("sample set keys:")
    for k in sample_sets.keys():
        print(k)
    print()

    st = sorted(set([k[1] for k in sample_sets.keys()]))
    print(st)

    ts_neand1 = ts.simplify(
        samples=sample_sets[(0, st[3])]
        + sample_sets[(1, st[3])]
        + sample_sets[(1, st[2])]
    )
    ...
    print("tree sequence sample times")
    for ts_sub in [ts_neand1, ts_neand2, ts_human]:
    	print(set([ts_sub.node(s).time for s in ts_neand1.samples()]))
    ...
    print(np.mean(num_seg_neand1), np.mean(num_intro_neand1))
    ...
    print(np.mean(num_seg_neand2), np.mean(num_intro_neand2))
    ...
    print(np.mean(num_seg_human), np.mean(num_intro_human))
    ...
    print("are introgressed fractions greater than zero?")
    print(np.mean(frac_intro_neand1), np.mean(frac_intro_neand2), np.mean(frac_intro_human))
    ...
    fname = f"ancestry_data.mean_{mean}.shape_{shape_param}.mu_{mu}.seed_{seed}.pkl.gz"
    ...
    print("window data")
    print(window_data)
    fname = f"window_ancestry_data.mean_{mean}.shape_{shape_param}.mu_{mu}.seed_{seed}.pkl.gz"
    with gzip.open(fname, "wb+") as fout:
    ...
    fname = f"recorder.mean_{mean}.shape_{shape_param}.mu_{mu}.seed_{seed}.pkl.gz"
    with gzip.open(fname, "wb+") as fout:
