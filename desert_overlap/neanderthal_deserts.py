...
def run_sim(
    L, g, mu, spacing=2e6, win_size=100000, SD=0.02, optimum=0.0, VS=1, burnin=10
):
    ...
    sample_times = [simlen - 250000 // 25, simlen - 120000 // 25, simlen - 45000 // 25]
...
if __name__ == "__main__":
    ...
    mu = float(sys.argv[2])
    SD = float(sys.argv[3])
    print(f"Running mu={mu}, SD={SD}, with seed={seed}")
    ...
    pop, recorder = run_sim(L, g, mu, optimum=optimum, VS=VS, SD=SD)
    ...
    ts_neand1 = ts.simplify(
        samples=sample_sets[(0, 10000.0)]
        + sample_sets[(1, 10000.0)]
    ...
    fname = f"ancestry_data.SD_{SD}.mu_{mu}.seed_{seed}.pkl.gz"
    ...
    fname = f"window_ancestry_data.SD_{SD}.mu_{mu}.seed_{seed}.pkl.gz"
    with gzip.open(fname, "wb+") as fout:
    ...
    fname = f"recorder.SD_{SD}.mu_{mu}.seed_{seed}.pkl.gz"
    with gzip.open(fname, "wb+") as fout:
