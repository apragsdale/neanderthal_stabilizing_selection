...
tar = tarfile.open(f"window_ancestry_data.mean_{mean}.shape_{shape}.mu_{mu}.tar.gz")
...
for i, m in enumerate(tar):
    ...
    human[ii] = data["Human"]
    neand1[ii] = data["Neand1"]
    neand2[ii] = data["Neand2"]
    ii += 1
...
x0, x1 = ax1.get_xlim()
ax1.hlines(0.02, x0, x1, linestyle="--", lw=1, color=colors[0], label=None)
ax1.hlines(0.05, x0, x1, linestyle="--", lw=1, color=colors[1], label=None)
...
fig.suptitle(
    f"Directional selection, with mean$={mean}$, shape$={shape}$, and $\\mu={0.01}$",
    fontsize=8,
    y=0.98,
)
fig.tight_layout()
...
plt.savefig(
    f"../figures/introgression_deserts.directional.mean_{mean}.shape_{shape}.pdf"
)
# plt.show()
