...
trait optimum is kept at $0$ in all branches, and the strength of selection
...
\citep{peter2016admixture}. This result is known
...
is zero), and $L$ is the number of trait affecting loci.
...
with free recombination between loci (Appendix~AY), we find that this provides
an excellent fit to average observed $V_G$ over time
...
mutations that were already segregating in the recipient population, those that
were introduced through introgression, and new mutations since the time of
admixture.

Paragraph on the following (point to appendix and supplement):
\begin{itemize}
