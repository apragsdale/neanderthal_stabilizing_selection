...
def new_mutations_2(freqs2, es, ss, mu, Nes, a):
...
def evolve_2(freqs2, es, ss, a, optima, VSs, mu, Nes, allele_labels=None):
    assert len(Nes) == 2
    assert len(optima) == 2
    assert len(VSs) == 2
    ...
    # introduce new mutations
    freqs2, es, ss = new_mutations_2(freqs2, es, ss, mu, Nes, a)
...
if __name__ == "__main__":
    ...
    for i in range(20000):
        freqs2, es, ss = evolve_2(
            freqs2, es, ss, a, [optimum, optimum], [VS, VS], mu, [Ne, Ne // 10]
        )
    ...
    VGs = np.array(VGs).T
    np.save(f"simulated_VGs/VGs_{out_idx}.a_{a}", VGs)
